...
from geonova_depthai.fence_linearization import (  # noqa: E402
    ...
    parse_args as parse_fence_args,
)
...
def test_default_entrypoint_configs_are_parseable() -> None:
    yolo = parse_yolo_args([], default_config=DEFAULT_CONFIG)
    fence = parse_fence_args([])

    assert yolo.config == DEFAULT_CONFIG.resolve()
    assert isinstance(yolo.dataset, Path)
    assert isinstance(fence.points, Path)
