...
제외하기 위해 `start_frame`은 200 이상이어야 합니다. 저장소의 `code`
디렉터리에서 인자 없이 실행하면 실행 파일과 같은 위치의 `tests/config.yaml`을
자동으로 읽습니다. 데이터셋·모델·출력 경로와 추론·선형화 파라미터는 이 YAML에서
관리합니다.
...
다른 설정 파일을 사용하려면 `--config`로 명시합니다. 이 경우 자동 설정 대신
지정한 YAML을 읽으며, 명시적인 CLI 옵션은 선택된 YAML 값을 덮어씁니다.
...
재실행할 수 있습니다. `code` 디렉터리에서 인자 없이 실행하면 모듈과 같은
위치의 `geonova_depthai/config.yaml`을 자동으로 읽습니다.

```bash
.venv/bin/python -m geonova_depthai.fence_linearization
```

이 명령은 YOLO 결과 폴더의 `points.csv`가 먼저 생성되어 있어야 합니다. 파일이
없다면 앞 절의 YOLO 일괄 실행을 먼저 수행하세요. 다른 설정으로 재실행할 때는
다음처럼 명시적으로 선택할 수 있습니다.

```bash
.venv/bin/python -m geonova_depthai.fence_linearization \
  --config configs/linearization.yaml
```
...
YOLO 일괄 실행과 방호 울타리 선형화는 예외적으로 인자 없이 실행해도 각각
`tests/config.yaml`, `geonova_depthai/config.yaml`을 자동으로 읽습니다. 자동
설정의 상대 경로가 `code` 기준이므로 아래 명령은 `code` 디렉터리에서
실행하세요. `--config`를 명시하면 해당 파일이 자동 설정보다 우선합니다.
